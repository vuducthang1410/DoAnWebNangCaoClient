...
const SignUpPage = () => {
  ...
  const validSignUp = () => {
    return (
      Utils.validataSignUp(user) &&
      Utils.regexValidEmail(user.email) &&
      Utils.repeatPassword(user.password, user.repeatPassword)
    );
  };
  ...
  const handleSignUp = async () => {
    if (validSignUp) {
      try {
        const reponse = await axios.post(url.url_sign_up, user);
  ...
  return (
    <div className="w-full containers">
      <div className=" flex items-center h-auto box-signup m-2 w-96 rounded-md ">
        <div className="flex flex-col justify-center items-center box-header-logo mb-8">
          <img
            className="h-14 m-6 image-logo translate-y-5"
            src={logo.url_logo}
            alt="hygge"
          />
          <label className="text-center text-base font-bold w-80 text-gray-500 m-2">
        ...
        <div>
          <button
            className="w-72 h-10 mt-3 bg-sky-400 rounded-md font-bold text-base text-white"
            name="btnLogin"
        ...
        <div className="flex justify-center items-center">
          ...
          <span className="m-1 my-2 text-slate-500 font-semibold text-sm">
            OR
          </span>
          <div className="h-px bg-slate-400 w-28"></div>
        </div>
        <button className="flex justify-center items-center bg-sky-400 w-72 h-10 rounded-md font-bold text-base text-white m-2">
          <svg
        ...
        <div className="mt-4 mb-8">
          <label href="">Have a account?</label>
          <a className="text-sky-400 ml-2" href="/login">
            Log in
          </a>
        </div>
      </div>
